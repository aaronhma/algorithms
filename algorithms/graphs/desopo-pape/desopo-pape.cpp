...
vi desopoPape(vector<vpii> &graph, vi &parent, int source, int n) {
  vi distance(n, INF_INT); // distance[i] - Distance from source -> i
...
int main()
{
  ...
  vi parent(n, -1);
  vi distance = desopoPape(graph, parent, 0, n);
