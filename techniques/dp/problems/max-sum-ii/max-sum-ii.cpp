...
int main()
{
  ...
  if (n <= 2) {
    // If the array is empty, we don't select anything.
    if (n == 0) cout << "0";

    // If the array only has one element, print that element.
    if (n == 1) cout << arr[0];

    // If the array has two elements, we only take the max element.
  ...
  FORE(i, 2, n) {
    // If we don't take arr[i], the answer is dp[i - 1].
    // If we take arr[i], the answer is dp[i - 2] + arr[i].
    // dp[i] = max of the two cases
